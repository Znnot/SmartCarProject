...
void My_ips200_draw_bianxain(void)
{
    uint16 temp1=0;
    uint16 temp2=0;
    uint16 temp3=0;
    for(int Ysite=LCDH-1;Ysite>ImageStatus.OFFLine;Ysite--)
    {
        temp1=2*ImageDeal[Ysite].LeftBorder;
        temp2=2*ImageDeal[Ysite].RightBorder;
        temp3=(temp1+temp2)/2;
        ips200_draw_point(temp1,Ysite*2,RGB565_RED);
        ips200_draw_point(temp1,Ysite*2,RGB565_BLUE);
        ips200_draw_point(temp1,Ysite*2,RGB565_GREEN);
    }
}
...
void my_ips200_show_gray_image (uint16 x, uint16 y, const uint8 *image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height, uint8 threshold)
{
    // 如果程序在输出了断言信息 并且提示出错位置在这里
    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
    zf_assert(x < ips200_width_max);
    ...
    for(i=0;i<LCDH;i++)
    {
//        ips200_draw_point(right_disline[i][0],right_disline[i][1],RGB565_RED);
//        ips200_draw_point(left_disline[i][0],right_disline[i][1],RGB565_RED);
//        ips200_draw_point(mid_disline[i][0],mid_disline[i][1],RGB565_BLUE);
        ips200_draw_rgb_point(right_disline[i][0],right_disline[i][1],RED);
        ips200_draw_rgb_point(left_disline[i][0],right_disline[i][1],RED);
        ips200_draw_rgb_point(mid_disline[i][0],mid_disline[i][1],BLUE);
    }
    if(IPS200_TYPE_SPI == ips200_display_type)
    {
        IPS200_CS(1);
